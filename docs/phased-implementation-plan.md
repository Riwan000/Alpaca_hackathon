...
- [x] **P5-BE-7** — Risk-engine aggregator → structured pass/fail with per-check reasons.
  ...
  - [x] Confirm — `backend/agents/risk/aggregate.py::aggregate_risk` runs the `DEFAULT_CHECKS` registry — all eleven P5-BE-1..6 checks (hedge budget, the P5-BE-2 trio, buying power, liquidity, contract validity, expiration window, net-delta bounds, multi-leg consistency, price band) — over one hypothesis and folds the `CheckOutcome`s into `AggregateRiskResult`: `.checks` is the full `RiskCheck` list (every check, pass or fail, each with its `detail` reason), `.verdict` is `APPROVE` only when all pass and `REJECT` the moment one hard-fails, `.violation_codes` the distinct `ViolationCode` per breach, `.warnings` the soft `CheckOutcome(warning=True)` details (e.g. a wide-but-tradeable spread), `.summary()` the tally. `aggregate_risk` threads `limits` only to checks whose signature accepts it (the P5-BE-1/2 four); the P5-BE-3..6 checks run with their own defaults. `test_confirm_full_checklist_visible_on_a_reject` shows a reject with the still-passing rows intact — a checklist, not a bare boolean — and a check may return one outcome or an iterable, both flattened in order.
- [x] **P5-BE-8** — Risk Agent — `APPROVE` / `MODIFY` / `REJECT`; cannot override a deterministic fail, cannot invent a strategy.
  ...
  - [x] Confirm — `backend/agents/risk/agent.py::RiskAgent.review` runs `aggregate_risk` first; on any hard fail it returns `verdict=REJECT` with the deterministic `violations` and **never calls the LLM** (`test_deterministic_fail_forces_reject_even_when_llm_approves` injects an approving stub + a 40k over-budget plan → REJECT, `mock_llm.calls == []`). On a clean plan the LLM returns `APPROVE` / `MODIFY` / `REJECT`; `approved_hypothesis` is always the incoming hypothesis so `strategy` is immutable by construction (a `strategy`/`strategy_type` response key or a modification targeting it is dropped — `test_llm_cannot_change_the_strategy_family`, `test_a_modification_that_swaps_strategy_is_dropped`), a `MODIFY` with no usable adjustment falls back to `APPROVE`, and any LLM failure (no key / unparsable) falls back to a deterministic `APPROVE` since every hard limit already cleared. `risk` prompt template added to `templates.yaml`.
- [x] **P5-BE-9** — `RiskDecision` output + `risk_checks` persistence.
  - Test: `tests/agents/test_risk_agent.py::test_persist` — decision validates as `RiskDecision`; a `risk_checks` row is written.
  - [x] Confirm — `RiskAgent.review(..., repo=)` calls `backend/agents/risk/agent.py::persist_risk_decision`, which maps the `RiskDecision` onto one `RiskCheckRecord` (full `checks` jsonb + `violations` / `warnings` / `modifications`, empty lists → `NULL` so the repo's REJECT→≥1-violation / MODIFY→≥1-modification / APPROVE→no-violation invariants line up) and writes exactly one row. `test_persist` validates `RiskDecision.model_validate(decision.model_dump())`, asserts `risk_repo.count() == 1`, that the full `checks` list and `warnings` round-trip, and runs `select cycle_id, verdict from risk_checks order by id desc limit 1`.
- [x] **P5-BE-10** — Execution Agent — build `ExecutionPlan` from an approved decision.
  - Test: `tests/agents/test_execution_plan.py` — plan legs match the approved hypothesis; MODIFY adjustments applied.
  - [x] Confirm — `backend/agents/execution/plan.py::build_execution_plan` refuses a `REJECT` (or hypothesis-less) `RiskDecision` with `ExecutionPlanError`; for `APPROVE`/`MODIFY` it copies `approved_hypothesis.legs` verbatim into an `ExecutionPlan` (`order_class` = `MLEG` for >1 leg, else `SINGLE`; `constraints.limit_price` = net combo debit `Σ ±limit_price`). Each `RiskModification` is applied by field — `quantity` (all legs) / `legs[i].quantity` / `limit_price` / `legs[i].limit_price` / `price_tolerance_pct` / `allow_legging`; an unknown field raises rather than being dropped. `estimated_cost` = `hypothesis.cost` scaled by the contract-count ratio when a quantity mod resized the structure. `test_modify_quantity_is_applied_and_cost_scaled` cuts 2→1 per leg → every leg `quantity == 1`, cost `240 → 120`.
- [x] **P5-BE-11** — Pre-flight validation before submit (re-check contract, price, buying power).
  - Test: `tests/agents/test_preflight.py` — stale quote or drifted price since approval → abort before submit, no order sent.
  - [x] Confirm — `backend/agents/execution/preflight.py::run_preflight` re-checks the plan against a fresh `HedgeContext`, first failure wins: leg no longer in the candidate set (`UNKNOWN_CONTRACT`) or expired as of `now` (`CONTRACT_EXPIRED`); context older than `max_quote_age` (default 5 min → `STALE_QUOTE`); any leg's approved `limit_price` more than `constraints.price_tolerance_pct` from the current `_match.candidate_mid` (`PRICE_DRIFT`); `estimated_cost` over `portfolio_state.buying_power` (`INSUFFICIENT_BUYING_POWER`). A non-`ok` `PreflightResult` carries the code + reason; the `/execute` caller writes an `execution_failures` row and never submits. `test_price_drift_since_approval_aborts_before_submit` moves the 145-put mid to 4.10 → `PRICE_DRIFT`; `test_execute_preflight_abort_sends_no_order` asserts `orders` stays empty and the broker was never called.
- [x] **P5-BE-12** — Multi-leg order submission via Alpaca (combo order preferred over independent legs).
  ...
  - [x] Confirm — `backend/integrations/alpaca/orders.py::build_mleg_order_payload` renders one `order_class: "mleg"` body — `qty` = GCD of the leg contract counts, per-leg `ratio_qty` = count ÷ GCD, `position_intent` opening, combo `limit_price` from `constraints`. `submit_plan(client, plan)` POSTs that single order via `AlpacaClient.submit_order` (new `_post` on the client); it only calls `build_single_leg_payloads` and submits leg-by-leg when the broker rejects the combo with an "unsupported / not eligible / mleg" `AlpacaError` **and** `constraints.allow_legging` (or the `allow_legging=` arg) is set — an unrelated error (e.g. "insufficient buying power") propagates untouched. `test_submit_plan_legs_in_only_when_combo_unsupported_and_allowed` → 1 rejected combo + 2 leg orders; `test_submit_plan_does_not_leg_in_when_legging_disallowed` → the `AlpacaError` propagates, no retry. `@pytest.mark.smoke test_smoke_submits_two_leg_paper_combo` pulls a live SPY put chain and submits a real 2-leg combo.
- [x] **P5-BE-13** — Legging-risk / partial-fill recovery path; never report false success.
  ...
  - [x] Confirm — `backend/agents/execution/result.py::build_execution_result` reads per-leg `filled_qty` / `filled_avg_price`: any unfilled or short leg → `status=PARTIALLY_FILLED` (never `FILLED`), the unfilled leg in `failed_legs`, and `recovery_action` recorded on the new `ExecutionResult.recovery_action` field — `UNWIND_FILLED_LEGS` when a filled short leg is left naked by an unfilled long leg, else `CANCEL_UNFILLED_LEGS`. A broker that claims `filled` while a leg shows no fill is still downgraded — the fills are the source of truth. `test_one_leg_fills_one_does_not_is_partially_filled` and `test_naked_short_leg_recovery_is_unwind_not_cancel` cover both recovery branches; `test_clean_fill_reports_no_recovery_needed` → `FILLED`, `recovery_action == "NONE"`.
- [x] **P5-BE-14** — `ExecutionResult` mapping (`FILLED` / `PARTIALLY_FILLED` / `FAILED` / `CANCELLED`) + fills persistence.
  - Test: `tests/agents/test_execution_result.py` — each Alpaca terminal state maps to the right enum; fills written with prices.
  - [x] Confirm — `result.py::map_broker_status` maps `filled`→`FILLED`, `partially_filled`→`PARTIALLY_FILLED`, `canceled`/`cancelled`/`expired`→`CANCELLED`, `rejected`/`suspended`/`stopped`→`FAILED`; a live state (`new`/`accepted`/…) or an unknown token raises `ExecutionResultError` rather than fabricating a terminal result. `build_execution_result` fills `filled_legs` (realized `price`, `slippage = price − plan limit`), `actual_cost` (`|Σ ±price·qty·100|`) and the mean `slippage`. `persist_execution_result` writes one `orders` row (`ExecutionStatus` mapped onto the lifecycle enum, `FAILED`→`REJECTED`) plus one `fills` row per filled leg in a single `save_with_fills` transaction. `test_persist_execution_result_writes_order_and_fills` → 1 order (`FILLED`, broker id set) + 2 fills with prices `[1.90, 3.20]`.
...
- [x] **P6-DB-2** — Persist node transitions so a restart resumes the decision history (Tech-Stack §21).
  ...
  - [x] Confirm — `backend/agents/orchestrator/runner.py::CycleRunner` drives `INITIAL → ANALYZING → STRATEGY_EVALUATION → RISK_CHECK → EXECUTION → MONITORING`, checkpointing each node to `workflow_state` / `workflow_transitions` (`WorkflowRepository.set_state`) the moment its body returns. `run(cycle_id)` reads the persisted state and resumes at the node *after* `current_node` (`_resume_index`); a terminal (`COMPLETED` / `FAILED`) cycle is returned untouched, and `KeyboardInterrupt` / `SystemExit` records the last finished node (or `INITIAL` for a brand-new cycle killed before any node finished) as `HALTED` and re-raises, so the cycle is never orphaned. `run()` with no id calls the new `WorkflowRepository.latest_unfinished_cycle()` to pick the interrupted cycle back up on a cold start; a typo in an injected node name is rejected rather than silently falling back to the no-op. Checkpoint-*after*-body means a side-effecting node (`EXECUTION`) re-enters on a kill inside its body, so it must self-guard on its own persisted effect — `OrderRepository.list_for_cycle` before submitting (not `CycleContext.already_ran`, which only ever reports *upstream* nodes) — so a resume never doubles an order. `tests/e2e/test_resume.py` (7 tests): Ctrl-C in `ANALYZING` → resume completes without replaying `INITIAL`, one order; kill right after `EXECUTION` submits but before its checkpoint → resume's guard sees the order, still one; restart with no cycle id resumes the halted one; kill inside `INITIAL` still leaves a resumable cycle; default-node run persists every transition; unknown node name rejected; a finished cycle re-runs clean.
...
- [x] **P6-BE-2** — Node: `ANALYZING` wraps the Phase 3 chain and builds `HedgeContext`.
  - Test: `tests/agents/test_node_analyzing.py` — node output state carries a valid `HedgeContext`; agent failure sets `degraded`.
  - [x] Confirm — `backend/agents/orchestrator/nodes.py::analyzing_node` runs `assemble_hedge_context` over the injected `inputs_provider` (default `build_live_analysis_inputs`), threading the LLM client and — when an engine is configured — an `AgentRunRepository` for `agent_runs` rows. It writes `hedge_context` onto the state and keeps the state's `cycle_id` authoritative (`inputs.model_copy` when they differ). A per-agent failure is already absorbed by the assembler → `degraded_sections` non-empty → the node sets `degraded=True` + a `notes` entry; a *hard* failure (inputs provider raising) is an `errors` entry + `route=MONITORING`, never a crash. `tests/agents/test_node_analyzing.py` (4 cases) green; end-to-end wiring in `tests/agents/test_graph_wired.py`.
- [x] **P6-BE-3** — Node: `STRATEGY_EVALUATION` wraps Phase 4.
  ...
  - [x] Confirm — `nodes.py::strategy_evaluation_node` runs the four hedge-family agents → `prefilter` → `StrategyManager(client=…).run`, writes `strategy_decision`, and sets `route` = `MONITORING` for a `NO_TRADE` / `REASSESS` outcome (nothing to risk-check) else `RISK_CHECK`. Missing context is an `errors` entry routed to `MONITORING`. The pure `route_after_strategy(state)` returns the same next node (P6-BE-7 attaches it as a conditional edge). `tests/agents/test_node_strategy.py` (3 cases) green.
- [x] **P6-BE-4** — Node: `RISK_CHECK` wraps the Phase 5 risk gate.
  - Test: `tests/agents/test_node_risk.py` — REJECT routes to `MONITORING` (no execution); APPROVE/MODIFY routes to `EXECUTION`.
  - [x] Confirm — `nodes.py::risk_check_node` calls `RiskAgent(client=…).review(selected_hypothesis, ctx, repo=RiskCheckRepository)` — the single `risk_checks` row is written here (the `EXECUTION` node deliberately skips step 2 of `POST /execute` so it is not double-written). `route` = `EXECUTION` only on `APPROVE` / `MODIFY`, else `MONITORING`; a `NO_TRADE` decision (no hypothesis) is a routed no-op. Fed straight into `execution_node`, a `REJECT` decision hits the verdict guard and submits nothing (`OrderRepository.count() == 0`). `route_after_risk(state)` mirrors it. `tests/agents/test_node_risk.py` (3 cases) green.
- [x] **P6-BE-5** — Node: `EXECUTION` wraps Phase 5 execution.
  - Test: `tests/agents/test_node_execution.py` — writes `ExecutionResult` to state; `FAILED` recorded, not swallowed.
  - [x] Confirm — `nodes.py::execution_node` guards (only `APPROVE` / `MODIFY` + an approved hypothesis; never a second order for a cycle that already has one — `OrderRepository.list_for_cycle`), then mirrors `POST /execute`: `build_execution_plan` → `run_preflight` (`deps.preflight`, on by default) → `submit_plan` → `build_execution_result` → `persist_execution_result`. Persistence splits on *when* a failure happened: before the broker is touched (bad plan, pre-flight abort, no broker, a rejected submission) → an `execution_failures` row only, and the cycle stays retryable; once `submit_plan` returns the order is live, so an `orders` row is written unconditionally — a `FAILED`/unmappable result lands as `REJECTED` with the broker id — so the idempotency guard trips on a re-invoke. Every failure is a truthful `FAILED` `ExecutionResult` on the state **and** an `errors` entry; nothing is raised out of the node. `tests/agents/test_node_execution.py` (5 cases) green: filled path persists an `orders` row; `AlpacaError` → `FAILED` not swallowed; stale context aborts at pre-flight with no order; broker-accepts-but-unmappable still writes the `orders` row and blocks a re-submit; re-invoke does not double-submit.
