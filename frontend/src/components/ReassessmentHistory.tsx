...
export const ReassessmentHistory: React.FC<ReassessmentHistoryProps> = ({
  ...
  if (isLoading) {
    return (
      <div data-testid="reassessment-history-loading" className="p-4 bg-slate-900 border border-slate-800 rounded-lg animate-pulse">
  ...
  return (
    <div data-testid="reassessment-history" className="bg-slate-900 border border-slate-800 rounded-lg p-5">
      <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
        <div>
          <h3 className="text-base font-semibold text-white">Reassessment & Adaptation History</h3>
          <p className="text-xs text-slate-400">Level-2 intelligent reassessments linking triggers to hedge ratio adjustments</p>
        </div>
        <span className="text-xs font-mono text-slate-500">{reassessments.length} Record{reassessments.length === 1 ? '' : 's'}</span>
      </div>

      {reassessments.length === 0 ? (
        <div data-testid="empty-reassessments" className="p-6 text-center text-slate-500 text-sm border border-dashed border-slate-800 rounded-md">
        ...
        <div className="space-y-3">
          {reassessments.map((item) => (
            <div
              ...
              className="p-4 rounded-lg bg-slate-950/70 border border-slate-800 hover:border-slate-700 transition-colors"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-slate-600 font-mono">({item.cycle_id})</span>
                </div>
              ...
              <p className="text-xs text-slate-300 mb-1">{item.reason}</p>
